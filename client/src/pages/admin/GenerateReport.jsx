...
const GenerateReport = () => {
  ...
  useEffect(() => {
    const fetchAllInvoice = async () => {
      try {
        // all-invoice
        const res = await axios.get(`${apiURL}/all-invoice`);
  ...
  useEffect(() => {
    const fetchTotalInvoice = async () => {
      try {
        // all-invoice
        const res = await axios.get(`${apiURL}/total-earnings`);
        //"${apiURL}/commission" - local computer
        //"http://192.168.1.47:8800/commission" - netwrok
  ...
  const filterErrands = invoices.filter((invoice) => {
    ...
    const employerFullName =
      `${invoice.employerFirstName} ${invoice.employerLastName}`.toLowerCase();
    const catcherFullName =
      `${invoice.catcherFirstName} ${invoice.catcherLastName}`.toLowerCase();

    const termMatchFullName =
      employerFullName.includes(searchTerm.term.toLowerCase() ?? "") ||
    ...
    return (
      (termMatch || termMatch2 || termMatchFullName) &&
      type &&
      priceMatches &&
      searchDate &&
      searchMonth &&
    ...
    <div ref={contentRef}>
      <div className="commissions">
        ...
        >
          ...
          <div class="col-md-4 col-xl-3 mb-3">
            <div
              class="card bg-c-green order-card text-center"
              style={{ height: "200px" }}
            >
          ...
          <div class="col-md-4 col-xl-3 mb-3">
            ...
            >
              <div class="card-block">
                ...
                <h2 class="text-center">
                  <i class="fa fa-cart-plus f-left"></i>
                  <span>₱{(totalInvoice / 100).toFixed(2)}</span>
                </h2>
      ...
      <GeneratePDF contentRef={contentRef} buttonLabel="Generate PDF" />
    </div>
  );
};
